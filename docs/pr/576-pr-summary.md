...
* Add regression coverage for toolbar delivery, Control-modified characters, and local overlay monitor routing.
...
* **Focused regression suite:** `./scripts/run-tests.sh -only-testing:SnapzyTests/RecordingAnnotationStateTests` — 13 tests passed.
...
* **Covered edge cases:** toolbar-focused key delivery, overlay local-monitor delivery, and Control-modified input resolved through `charactersIgnoringModifiers`.
